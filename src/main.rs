...
use llr_reco::cuda::cufft::{fft3c_chunked, ifft3c_chunked};
...
fn main() {
    let data_set_dir = Path::new("C:/Users/wa41/llr-reco-test-data");
    let test_outputs = Path::new("C:/Users/wa41/llr-reco-test-data/out");
    ...
    let data_set_size = [512, 512, 512, 67];
    let block_size = [30, 30, 30];
    let max_fft_vols_per_batch = 10;
    let max_matrices_per_batch = 500;
    let max_random_shift = [256, 256, 256];

    let matrix_size: [usize; 2] = [block_size.iter().product(), n_vols];
    ...
    println!("allocating data set ({} GB) ...", total_bytes as f64 / 2f64.powi(30));
    ...
    data.par_chunks_exact_mut(vol_stride).enumerate().for_each(|(i, vol)| {
        let volname = format!("vol{:02}", i);
        let x = cfl::to_array(data_set_dir.join(volname), true);
    ...
    let vol = data_iterate.index_axis(Axis(3), 10).to_owned();
    cfl::dump_magnitude(test_outputs.join("test_vol_pre"), &vol.into_dyn());
    ...
    for it in 0..n_iter {
        ...
        println!("current error: {:e}", relative_iterate_error);
